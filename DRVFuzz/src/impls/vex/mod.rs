...
pub use execute::{
    ExceptionEvent, ExecutorConfig, MemoryWrite, RegisterWrite, TraceConfig, VexExecutor,
};
...
pub fn execute_with_extension_override<P: AsRef<Path>>(
    ...
    let mut config = ExecutorConfig::new(run_root.as_ref().to_path_buf(), isa_base, RiscVImpl::Vex)
    ...
    let executor = VexExecutor::new(config);
