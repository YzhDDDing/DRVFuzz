...
pub use execute::{
    ExceptionEvent, ExecutorConfig, IbexExecutor, MemoryWrite, RegisterWrite, TraceConfig,
};
...
pub fn execute<P: AsRef<Path>>(
    ...
    let mut config =
        ExecutorConfig::new(run_root.as_ref().to_path_buf(), isa_base, RiscVImpl::Ibex)
            .map_err(ImplExecutionError::from)?;
    config.timeout = timeout;

    let executor = IbexExecutor::new(config);
