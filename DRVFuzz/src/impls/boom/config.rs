...
/// BOOM v3 supported ISA extensions.
pub(crate) fn extensions_v3() -> ExtensionMap {
    ExtensionMap {
        rv32: vec![
            RV32Extensions::I,
            RV32Extensions::M,
...
/// BOOM v4 supported ISA extensions.
///
/// Compared to v3, v4 additionally supports Zba/Zbb/Zbs.
pub(crate) fn extensions_v4() -> ExtensionMap {
    let mut map = extensions_v3();
    map.rv64.extend([
        RV64Extensions::Zba,
        RV64Extensions::Zbb,
        RV64Extensions::Zbs,
...
exit_s_mode:
    ...
    ret                          # Handler will adjust mepc to skip this
"#
            .to_string();
            ("sd", ".dword  0", block)
        }
...
_start:
    ...
    # Open up PMP: a single NAPOT covers the full address space, allowing S-mode fetch/load/store
    li      t0, -1
    csrw    pmpaddr0, t0
