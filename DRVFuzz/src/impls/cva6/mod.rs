...
pub use execute::{
    Cva6Executor, ExceptionEvent, ExceptionSource, ExecutorConfig, MemoryWrite, RegisterWrite,
    RvfiTrace, TraceConfig,
};
...
pub fn execute<P: AsRef<Path>>(
    ...
    let mut config =
        ExecutorConfig::new(run_root.as_ref().to_path_buf(), isa_base, RiscVImpl::CVA6)
            .map_err(ImplExecutionError::from)?;
    config.timeout = timeout;

    let executor = Cva6Executor::new(config);
