...
pub use execute::{
    ExceptionEvent, ExecutorConfig, MemoryWrite, RegisterWrite, Srv32Executor, Srv32Trace,
};
...
pub fn execute<P: AsRef<Path>>(
    ...
    let mut config =
        ExecutorConfig::new(run_root.as_ref().to_path_buf(), isa_base, RiscVImpl::Srv32)?;
    config.timeout = timeout;
    let executor = Srv32Executor::new(config);
